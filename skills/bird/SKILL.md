...
bird news --with-ai-summary    # Include Grok-written AI summaries for trends
bird trending                  # Alias for news
```

**--with-ai-summary**: Uses `AiTrendByRestId` API to fetch AI-generated summaries (Grok-written) for trends. This provides an AI-curated summary of the trend with a disclaimer that content may evolve.

```bash
bird news --ai-only -n 3 --with-ai-summary  # Get AI trends with Grok summaries
```
